...
에타(eta) 외에도 알파(alpha) 값이 보이는데, 이건 모멘텀 변수이다. 종전에 움직이던 기울기에 알파(여기선 90%)를 곱해 거의 유지해주고, 거기다가 학습률*그래디언트를 더해주는 방식이다.
...
첫 움직임에서는 크게크게 움직이고, 가면 갈수록 서서히 보폭(step size)을 줄이면서 찬찬히 minimum을 찾아가는 방식이 있다. 특히 더 크게 움직였을수록, 다음 스텝은 더욱 조금만 움직이도록 제어하는 방식이다.
...
그리고 이 커진 값에 루트를 씌워 그래디언트를 나눠주면, 비교적 컸던 그래디언트 원소는 더 큰 폭으로 줄어들게 되는 것이다. 즉 직전의 보폭이 클수록, 이번의 보폭은 더 큰 폭으로 줄이겠다는 의지가 반영된 식이다.
...
보통 기본 계산해서 감쇠율은 0.9나 0.99등을 사용하는데, 감쇠율을 작게 설정할수록 과거의 값은 잊혀지고 최신의 값들이 계산에 더 많이 반영되게 된다.

즉 끝없이 그래디언트를 기억하는 것이 아닌, 최신의 값들 위주로 반영함으로써 기울기가 0으로 사라지는 것을 방지한다. 이 같은 방식을 EMA(지수이동평균; Exponential Moving Average)라고 한다.
...
AdaGrad가 점 몇 개만에 기울기가 바로 줄어드는 것에 비해, RMSprop은 상대적으로 기울기가 완만히 줄어드는 것을 볼 수 있다
...
아담의 핵심은 저 beta1(1차 모멘텀용 계수)과 beta2(2차 모멘텀용 계수)라고 한다.
...
그런데 가중치를 최초에 전부 0으로 세팅하면은 또 안 된다. 정확힌 가중치가 다 같은 값을 띠면 안된다. 각 노드별로 차이가 생기지 않기에, 노드를 여러개를 둬도 하나로 합치는 것과 다를 바가 없기 때문이다.
...
Xavier 초깃값이 1 / sqrt(n)이었다면, He 초깃값은 <strong>2 / sqrt(n)</strong>이다. 코드부터 확인해보자.

```python
# 각 층이 동일한 5층짜리 딥러닝 모델
# 활성화함수는 ReLU
# 각각 가중치 w의 표준편차가 0.01, 1 / sqrt(n)(Xavier 초깃값), 2 / sqrt(n)(He 초깃값)일 때
