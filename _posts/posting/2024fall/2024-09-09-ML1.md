...
$$\mathcal{H}\;\text{shatters}\;S\subseteq \mathcal{J}\Leftrightarrow \forall \text{dichotomy}\;D\;\text{of}\;S[\exists h\in \mathcal{H}, \text{CONSISTENT(h,D)}\in \mathcal{H} ]$$

여기부턴 왜 굳이 수식으로 썼는지 이해가 안 될 정도이다. 무튼

객체 공간 J에 대한 모든 dichotomy가 가설 공간 H에서 shatter된다는 것은, consistent한 가설을 포함하는 가설 공간 H에 대하여, shatter한 공간 S에 모든 dichotomy D가 존재한다는 것이다. (아니 이게 맞아??? 이거 해석하는 법 누가 좀 알려주세요)
...
이 VC 차원이 n임은 (1) H에 의해 shattering되는 instance n개 이상이 존재함을 보이고 (2) n+1개는 불가능함을 보임으로 증명한다.
